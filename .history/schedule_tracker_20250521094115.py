...
# Handle KB
khad = next(row for row in rows if st.secrets["dropbox"]["refresh_token"] in row['Name'])
kh_suffix = extract_parentheses(khad['Shift'])
khad['Name'] = f"{khad['Name']} ({kh_suffix})"
khad['Shift'] = '8:30am - 5:30pm'
#KB is row 49

# Handle Marcos
marcos = next(row for row in rows if 'Marcos Mendez' in row['Name'])
