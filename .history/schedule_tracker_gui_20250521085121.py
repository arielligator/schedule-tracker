...
with counter:
    ...
    # ① apply Name/Team/Location/Weekday filters only
    df_base = dynamic.filter_df(except_filter="Time of Day")

    # ② build counts for each one-hour window
    hours  = list(range(24))
    counts = []
    for h in hours:
    ...
    # ③ format labels without leading zeros
    labels = [
        f"{window_start.strftime('%I %p').lstrip('0')}–{window_end.strftime('%I %p').lstrip('0')}"
        for window_start, window_end in [
            (time(h, 0), time((h + 1) % 24, 0)) for h in hours
        ]
    ]
