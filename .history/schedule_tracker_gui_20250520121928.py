...
# TIME COUNTER
with counter:
    # 1️⃣ Base DataFrame (name/team/loc/day filters only)
    df_base = dynamic.filter_df(except_filter="Time of Day")

    # 2️⃣ Build your hours_to_plot exactly as before…
    #    (Now / Custom Time / Custom Range / All)
    #    …and also capture single‐time `t` if you need the "On duty at…" line.

    # 3️⃣ Now compute counts using interval_overlaps:
    if hours_to_plot:
        counts = []
        for h in hours_to_plot:
            # define the full hour interval [h:00 → (h+1)%24:00)
            hr_start = time(h, 0)
            hr_end   = time((h + 1) % 24, 0)

            # count every row whose [Start→End) overlaps this hour window
            c = df_base.apply(
                lambda row: interval_overlaps(
                    parse_time(row["Start Time"]),
                    parse_time(row["End Time"]),
        ...
        # map into a Series and label "1 am", "2 am", etc.
        ...
        df_counts.index = df_counts.index.map(fmt)

        st.dataframe(df_counts, use_container_width=True)
    else:
        st.write("⏳ Enter a valid time (or range) above…")

    # 4️⃣ And if you want the “On duty at t” line, use on_duty() too:
    if t is not None:
