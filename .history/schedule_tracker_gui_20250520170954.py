...
# TIME COUNTER
with counter:
    # 0️⃣ Base DataFrame (apply Name/Team/Location/Day filters only)
    df_base = dynamic.filter_df(except_filter="Time of Day")

    # 1️⃣ Figure out which hours to plot — always initialize first!
    mode = st.session_state.get("time_mode", "All")
    hours_to_plot = []   # ← make sure this exists even if no branch matches
    t = None             # ← for your “On duty at t” line
    ...
    if mode == "Now":
        ...
        if start_txt and end_txt:
            try:
                s = parse_time(start_txt)
                e = parse_time(end_txt)
                # e.g. plot each whole hour between s.hour and e.hour
    ...
    # 2️⃣ Now it’s safe to do “if hours_to_plot: …”
    if hours_to_plot:
        # your interval_overlaps loop or whatever you use
        …
    else:
        st.write("⏳ Enter a valid time (or range) above…")

    # 3️⃣ And only if t was set do we do the single-time mask
    if t is not None:
