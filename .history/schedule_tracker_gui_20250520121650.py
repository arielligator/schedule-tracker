...
with counter:
    # ————————————————
    # 1️⃣ Base DataFrame (skip only the dynamic filters you want)
    df_base = dynamic.filter_df(except_filter="Time of Day")

    mode = st.session_state.get("time_mode", "All")
    t = None       # <— make sure it always exists
    # Decide which hours to plot and capture 't' when relevant
    if mode == "Now":
        t = datetime.now().time()
        ...
        if txt:
            try:
                t = parse_time(txt)
                hours_to_plot = [t.hour]
                st.success(f"✔ Counting at **{t.strftime('%I:%M %p').lstrip('0')}**")
            except ValueError:
                hours_to_plot = []
        else:
            hours_to_plot = []
        ...
        if start_txt and end_txt:
            try:
                s = parse_time(start_txt)
                e = parse_time(end_txt)
                # keep your range logic for the chart
                hours_to_plot = list(range(s.hour, e.hour))
                st.success(f"Range: **{s.strftime('%I:%M %p')}**–**{e.strftime('%I:%M %p')}**")
            except ValueError:
                hours_to_plot = []
        else:
            hours_to_plot = []
    ...
    # ————————————————
    # 2️⃣ Compute your hourly‐counts chart exactly as before
    if hours_to_plot:
        df_base["start_hour"] = df_base["Start Time"].apply(lambda s: parse_time(s).hour)
        df_base["end_hour"]   = df_base["End Time"].  apply(lambda s: parse_time(s).hour)

        counts = [
            ((df_base["start_hour"] <= h) & (df_base["end_hour"] > h)).sum()
    ...
    # ————————————————
    # 3️⃣ Only if we actually captured a single t, show the “on duty at t” mask
    if t is not None:
        mask = (
            df_base["Start Time"].apply(parse_time) <= t
        ) & (
            df_base["End Time"].  apply(parse_time)   >  t
        )
        st.text(f"On duty at {t.strftime('%I:%M %p').lstrip('0')}: {mask.sum()}")
